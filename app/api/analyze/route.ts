...
export async function POST(req: NextRequest) {
  ...
  // A chave pode vir do app (usuario) ou de variavel de ambiente no servidor.
  const apiKey =
  ...
  if (!apiKey) {
    return NextResponse.json(
      { error: "Informe a chave de API (no app ou em variavel de ambiente)." },
      { status: 401 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Falha ao contatar o provedor de IA. Verifique a chave e a conexao." },
      { status: 500 }
    );
  }
}
